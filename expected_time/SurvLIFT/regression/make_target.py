...
def process_seed(seed):
    ...
    beta_filename = f"beta_coefficients_seed{seed}.csv" # [New] 베타값 저장 파일명
    ...
    beta_path = os.path.join(project_root, "data", beta_filename) # [New]
    ...
    # 7. 결과 저장
    # [중요 변경] Log Scale 값을 Exponential 씌워서 원래 시간(Time)으로 복구
    df['target_log_time'] = new_targets_log # 참고용 로그 값
    
    # exp(log_T) -> T, 그리고 소수 둘째자리 반올림 (LLM 입력용)
...
def main():
    print("=== 전체 Seed 데이터에 대한 Target 생성 (Linear Scale) 및 Beta 저장 시작 ===")
    # Seed 1부터 10까지 반복
