...
def parse_number_4dec(text: str,
    ...
    #Optional[float]: 실수가 들어올 수도 있고 아닐 수도 있음
    # =은 디폴트 지정, :은 그냥 타입 알려주는 역할
    if stop_str and stop_str in text: # if object의 결과는 T/F이다
        text = text.split(stop_str, 1)[0] #1은 한번만 쪼개라 stop_str을 기준으로
